  ...
  async ({ params, body, user, set, headers, requestLogger }) => {
    ...
    try {
      ...
      if (supplier.supplierUserId) {
        throw new ApiError(
          400,
          "SUPPLIER_HAS_CONTACT",
          "This supplier already has a contact user. Use the edit contact feature to update existing contact."
        );
      }
      ...
      if (authError || !authUser.user) {
        ...
        throw new Error(
          `Failed to create Supabase user: ${authError?.message}`
        );
      }
