...
describe("Workflow Step Validation Config API", () => {
  ...
  test("create step with requiresValidation=true and valid approverRoles", async () => {
    const response = await client.api["workflow-templates"][template.id].steps.post({
  ...
  test("update step to add validation config", async () => {
    ...
    const createResponse = await client.api["workflow-templates"][template.id].steps.post({
    ...
    const updateResponse = await client.api["workflow-templates"][template.id].steps[stepId!].put({
      name: "Update Test Step",
      stepType: "document",
      requiresValidation: true,
      validationConfig: {
    ...
    expect(updateResponse.data?.data?.validationConfig.approverRoles).toEqual(["admin"]);
  });
  ...
  test("retrieve step and verify validation config persisted", async () => {
    ...
    const getResponse = await client.api["workflow-templates"][template.id].get({
      headers: {
        Authorization: `Bearer ${adminToken}`,
      },
    });
    ...
    const step = getResponse.data?.data?.steps?.find((s: any) => s.id === stepId);
