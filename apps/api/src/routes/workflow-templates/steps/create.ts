 ...
 */
export const createStepRoute = new Elysia().use(requireAdmin).post(
  "/:workflowId/steps",
    ...
    try {
      ...
      if (body.formTemplateId) {
        ...
        if (formTemplateRecord.status !== "published") {
          throw Errors.badRequest(
            "Only published form templates can be used in workflow templates",
            "FORM_TEMPLATE_NOT_PUBLISHED"
          );
        }
      }
      ...
      if (body.stepType === "form" && !body.formTemplateId) {
        throw Errors.badRequest(
          "A form template is required for form steps",
          "MISSING_FORM_TEMPLATE"
        );
      }
      if (body.stepType === "document" && !body.documentTemplateId) {
        throw Errors.badRequest(
          "A document template is required for document steps",
      ...
      if (body.requiresValidation) {
        if (
          !body.validationConfig?.approverRoles ||
          body.validationConfig.approverRoles.length === 0
        ) {
          throw Errors.badRequest(
            "When requiresValidation is true, validationConfig.approverRoles must be a non-empty array",
  ...
  },
  {
    ...
    body: t.Object({
      ...
      formActionMode: t.Optional(
        t.Nullable(t.Union([t.Literal("fill_out"), t.Literal("validate")]))
      ),
      documentTemplateId: t.Optional(t.String()),
      documentActionMode: t.Optional(
        t.Nullable(t.Union([t.Literal("upload"), t.Literal("validate")]))
      ),
