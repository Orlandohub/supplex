 ...
 */
export const listStepDocumentsRoute = new Elysia().use(authenticate).get(
  ...
  async ({ params, user }) => {
    ...
    if (workflowTemplateId) {
      ...
      if (stepTmpl?.documentTemplateId) {
        ...
        if (docTmpl?.requiredDocuments) {
          requiredDocsInfo =
            docTmpl.requiredDocuments as RequiredDocumentItem[];
        }
      }
    }
    ...
    if (currentRound > 0) {
      decisionRows = await db
        ...
        .innerJoin(
          taskInstance,
          eq(documentReviewDecision.taskInstanceId, taskInstance.id)
        )
        .where(
          and(
    ...
    for (const row of decisionRows) {
      const list = decisionsByDoc.get(row.workflowStepDocumentId);
      if (list) {
        list.push(row);
      } else {
        decisionsByDoc.set(row.workflowStepDocumentId, [row]);
      }
    }
