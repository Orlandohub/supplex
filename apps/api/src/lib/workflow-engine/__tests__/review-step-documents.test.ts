...
describe("reviewStepDocuments", () => {
  ...
  test("single reviewer approves all → decision rows created, step validated, transition", async () => {
    ...
    await db.delete(workflowStepTemplate).where(eq(workflowStepTemplate.id, step1Tmpl.id));
  ...
  test("any-declined → decision rows created with comments → step reset to active", async () => {
    ...
    expect(decisionRows.length).toBe(2);
    const approvedDecision = decisionRows.find((d) => d.decision === "approved");
    const declinedDecision = decisionRows.find((d) => d.decision === "declined");
    ...
    const taxDoc = docs.find((d) => d.requiredDocumentName === "Tax Certificate");
    const insDoc = docs.find((d) => d.requiredDocumentName === "Insurance Policy");
  ...
  test("CAS conflict / idempotency — second review with same task fails, no duplicate decisions", async () => {
    ...
    const result1 = await db.transaction(async (tx) => {
      return reviewStepDocuments(tx, {
        ...
        taskId: valTask.id,
        decisions: [
          { requiredDocumentName: "Certificate", action: "approve" },
        ],
      });
    });
  ...
  test("multi-approver: reviewer 1 approves → partial, reviewer 2 approves → final with transition", async () => {
    ...
    const [t1] = await db.select().from(taskInstance).where(eq(taskInstance.id, task1.id));
    const [t2] = await db.select().from(taskInstance).where(eq(taskInstance.id, task2.id));
    ...
    // Step validated
    const [stepAfterSecond] = await db
      .select()
      .from(stepInstance)
      .where(eq(stepInstance.id, stepInst.id));
  ...
  test("resubmission round isolation: old round decisions not consulted in new round", async () => {
    ...
    const declineResult = await db.transaction(async (tx) => {
      return reviewStepDocuments(tx, {
        ...
        decisions: [
          { requiredDocumentName: "Certificate", action: "decline", comment: "Bad quality" },
        ],
      });
    });
