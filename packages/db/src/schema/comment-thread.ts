...
export const commentThread = pgTable(
  'comment_thread',
  {
    ...
    processInstanceId: uuid('process_instance_id').notNull().references(() => processInstance.id, { onDelete: 'cascade' }),
    stepInstanceId: uuid('step_instance_id').notNull().references(() => stepInstance.id, { onDelete: 'cascade' }),
    entityType: varchar('entity_type', { length: 50 }).notNull(), // 'form' or 'document'
    entityId: uuid('entity_id'), // nullable - for future use
    parentCommentId: uuid('parent_comment_id').references((): any => commentThread.id, { onDelete: 'cascade' }),
    commentText: text('comment_text').notNull(),
    commentedBy: uuid('commented_by').notNull().references(() => users.id, { onDelete: 'restrict' }),
  ...
  (table) => ({
    ...
    parentIdx: index('idx_comment_thread_parent')
      ...
      .where(sql`${table.deletedAt} IS NULL AND ${table.parentCommentId} IS NOT NULL`),
